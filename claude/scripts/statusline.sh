...
# --- Branch + PR ---
# PR lookup hits the network, so it is cached per branch and refreshed in the
# background once the cache is older than 5 minutes.
# ponytail: cache never evicts; add find -mtime cleanup if /tmp fills up.
git_badge=""
branch=$(git -C "$cwd_full" branch --show-current 2>/dev/null)
if [ -n "$branch" ]; then
  cache="${TMPDIR:-/tmp}/cc-pr-$(printf '%s' "$cwd_full/$branch" | cksum | cut -d' ' -f1)"
  if [ ! -f "$cache" ] || [ -n "$(find "$cache" -mmin +5 2>/dev/null)" ]; then
    ( gh pr view "$branch" --repo "$(git -C "$cwd_full" remote get-url origin 2>/dev/null)" \
        --json number -q '"#\(.number)"' >"$cache" 2>/dev/null || : >"$cache" ) &
  fi
  pr=$(cat "$cache" 2>/dev/null)
  git_badge="${SEP}${GRAY}⎇ ${branch}${RESET}${pr:+ ${BOLD}${pr}${RESET}}"
fi
...
# --- Render ---
printf '%s' \
"${cwd}${git_badge}${SEP}${model_name}${effort_badge}${SEP}${bar} ${BOLD}${pct}%${RESET} ${DIM}(${total_h}/${limit_h})${RESET}"
